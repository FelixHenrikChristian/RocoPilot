...
1. 在“实时”页开启“奇遇统计”。
2. 首次使用前建议同步精灵图鉴数据。
3. 识别到当前赛季的奇遇解除信号后，工具会自动更新统计。
4. 识别到异色提示后，请前往“统计”页确认结果；确认后会写入异色记录，并重置对应精灵的当前赛季计数。
5. 需要备份或多设备同步统计数据时，可在“统计”页设置 Cloudflare R2 云同步，配置说明见 [Cloudflare R2 同步教程](docs/statistics-sync-cloudflare-r2.md)。
