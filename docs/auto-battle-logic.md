...
4. 如果战斗中这些子状态都未识别到，仍保持战斗态，并结束当前技能选择态。

战斗类型门禁（`AutoBattleType`：`Unknown` / `Normal` / `Boss` / `Legendary`）：

1. 进入战斗后类型先为 `Unknown`；遮罩显示「识别战斗类型中」，不把 `Unknown` 当成普通战斗。
2. 首回合技能选择界面经延迟 + 敌方名/首领名 OCR 后，才落定为 `Normal` 或 `Boss`（传说后续接入 `Legendary`）。
3. 类型未定时：不缓存释放步骤、不执行自动按键/换精灵；类型判定完成后再写对应遮罩并执行后续逻辑。
4. 技能选择分支先跑识别/动作，再刷新信息遮罩，避免先闪「战斗中」再跳「首领战斗」。
...
当前赛季如果使用 `EnemyNameTransition`，第一回合敌方名会影响技能释放等待时间：识别到占位名（例如“幸运惊喜盒”）时按普通 S2 流程快速释放；如果第一回合识别到的不是占位名，说明更像历史 S1 的 `battle-tip` 文字提示奇遇，本场战斗后续每次进入技能选择都会增加“污染奇遇额外等待”，让后台 OCR 有机会在提示文字出现较晚时先识别到奇遇解除，再决定是否按键。该值默认 `2500ms`，会叠加在默认 `500ms` 的技能选择基础等待上，两项都可在自动战斗的“其他配置”窗口调整。

技能选择界面出现时，处理流程如下：

1. 第一次识别到技能选择界面，只创建新回合状态，不立即按键。
2. 等待技能选择基础等待时间，默认 `500ms`，避免 UI 刚出现时按键丢失。
3. 选取当前释放步骤：
...
7. 如果补救后达到技能操作重试间隔仍停留在技能选择界面，会重试当前动作，默认间隔为 `4000ms`。

“其他配置”窗口还可以调整按键按下时长、普通按键间隔和捕捉序列按键间隔。已有设置没有这些字段时会采用默认值 `100ms`、`500ms` 和 `500ms`。
