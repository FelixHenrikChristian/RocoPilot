...
当前赛季如果使用 `EnemyNameTransition`，第一回合敌方名会影响技能释放等待时间：识别到占位名（例如“幸运惊喜盒”）时按普通 S2 流程快速释放；如果第一回合识别到的不是占位名，说明更像历史 S1 的 `battle-tip` 文字提示奇遇，本场战斗后续每次进入技能选择都会额外等待一小段时间，让后台 OCR 有机会在提示文字出现较晚时先识别到奇遇解除，再决定是否按键。
